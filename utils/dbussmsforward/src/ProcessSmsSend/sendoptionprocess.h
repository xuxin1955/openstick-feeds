...
void fordardSendSms(string sendMethodGuideResult, string telnum, string smscontent, string smsdate)
{
    ...
    // Record the history before dispatching to the channels. GetSmsCodeStr takes
    // the message body by value, so extracting the code here does not alter what
    // the channels receive.
    string SmsCode;
    string SmsCodeFrom;
    GetSmsCodeStr(smscontent, SmsCode, SmsCodeFrom);
    AppendSmsHistory(telnum, smscontent, smsdate, SmsCode, SmsCodeFrom);
